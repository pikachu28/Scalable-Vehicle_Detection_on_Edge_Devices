...
def draw_boxes_frame(frame, frame_size, boxes_dicts, class_names, input_size, name):
  ...
  for cls in range(len(class_names)):
    ...
    if np.size(boxes) != 0:
      for box in boxes:
        ...
        with open('Text/'+name+'.txt', 'a') as f:
          #f.write(xy[0] xy[1], xy[0] + test_width, xy[1] - text_height - baseline)
          f.write(str(xy[0])+' ')
          f.write(str(xy[2])+' ')
          f.write(str(xy[1])+' ')
          f.write(str(xy[3])+' ')
